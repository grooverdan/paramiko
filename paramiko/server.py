...
class ServerInterface (object):
    ...
    def check_channel_direct_unix_request(self, chanid, origin, destination):
        """
        Determine if a local unix forwarding channel will be granted, and
        return ``OPEN_SUCCEEDED`` or an error code.  This method is
        called in server mode when the client requests a channel, after
        authentication is complete.
        ...
        The origin parameters is a (ip_address, port) tuple that correspond to
        the remove ends the connection.

        The destination parameter corresponds to the the unix socket in the
        forwarding tunnel.
